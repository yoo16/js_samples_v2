...
`PoseLandmarker` は、`MediaPipe Tasks Vision` が提供する機能のひとつで、`Web` カメラの映像から全身の姿勢（ランドマーク）を推定します。`finger` フォルダの `HandLandmarker` と同じ仕組みで、対象が手から全身に変わったものです。
...
`finger` フォルダと同じく、`PoseLandmarker` も `Wasm`（`WebAssembly`）と学習済みモデルの `.task` ファイルを使います。どちらも `vendor` フォルダに同梱しており、インターネット接続がなくても動作します。
...
`Web` カメラを使うため、ローカルサーバー経由で開きます。全身が画面に映っていないと関節角度が正しく計算できないため、カメラから十分離れて、頭から足先まで映る位置で試してください。

お手本ポーズを切り替えながら実際にポーズを取り、スコアと骨格の色（緑・黄・赤）が姿勢に応じて変化するか確認します。「今の姿勢をお手本に保存」ボタンを押して、自分の姿勢がオリジナルのお手本として保存されるかも確認します。

## まとめ

| 項目 | 内容 |
| ---- | ---- |
| PoseLandmarker | 全身から33個のランドマークを検出する機能 |
| detectForVideo | 動画フレームからランドマークを推定するメソッド |
| POSE_CONNECTIONS | 骨格の線をつなぐための番号の組 |
| computeJointAngles | 3点の座標から関節の角度を算出する処理 |
| REFERENCE_POSES | お手本ポーズごとの目標角度と許容範囲の定義 |
| comparePose | 今の姿勢とお手本の角度差からスコアを算出する処理 |
